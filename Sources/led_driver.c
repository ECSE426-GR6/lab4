...
void Thread_LED (void const *argument) {
	...
	while(1){
		if (temp_mode){
			new_value = GLO_getTemperature();
		} else {
			if (target_was_set){
				new_value = GLO_getAngle();
			} else {
				new_value = GLO_get_KP_in();				
			}
		}

		if (new_value != current_value) {
			LED_set_value(new_value);
		}
	}
}
...
void LED_init_io (void){
	...
	LED_configuration.Pin	= GPIO_PIN_12|GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_6|GPIO_PIN_7|GPIO_PIN_11|GPIO_PIN_10|GPIO_PIN_9|GPIO_PIN_8|GPIO_PIN_13;
